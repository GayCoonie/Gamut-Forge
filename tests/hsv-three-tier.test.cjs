...
for(const suffix of ['64','10']){
 ...
 const key=suffix==='64'?'hsvThreeTier64':'hsvThreeTier10';assert.deepEqual(Array.from(ctx.window.GAMUT_PALETTES[key].colors),data.colors);
